...
def train_validate_classifier(clf, modelName, X, y, X_predict, y_predict, record_performance, sh_logger, modelDir,
                              n_classes, labels, type='classification'):
...
def grid_search(clf, modelName, params, X, y, X_predict, y_predict, record_performance, sh_logger, modelDir, n_classes,
...
def predict_classifier(clf, modelName, X_test, y_true, record_performance, sh_logger, n_classes, labels, modelDir):
    ...
    if sh_logger.info: print(
        f'Total Number of points: [{X_test.shape[0]}]  Mislabeled Points: [{(y_true != clf_pred).sum()}]')
